...
// Added by Jarno N. Alanko: a function that does nothing. Used as a default parameter for
// the callback in construction.
auto dummy_function = [](int_vector_buffer<>& sa, int_vector<8>& T) {return; }; 
...
public:
	...
	/*
	 * Build index
	 * Modified by Jarno N. Alanko 24.5.20: Now also takes a callback that is
	 * given the suffix array as a disk-streamable int_vector_buffer. The callback
	 * can be used to build extra data structures from suffix array.
	 */
	r_index(string &input, bool sais = true, std::function<void(int_vector_buffer<>& sa, int_vector<8>& T)> callback = dummy_function){
		...
		auto bwt_and_samples = sufsort(input, callback);
	...
	/*
	* Added by Jarno N. Alanko. Returns the largest index j < i such that
	* BWT[j] = c. If such j does not exist, returns -1
	*/
	long long bwt_pred(ulint i, char c){
		ulint rank = bwt.rank(i,c);
		if(rank == 0) return -1;
		return bwt.select(rank-1, uchar(c)); // -1 because rle_string select starts from 0
	}

	/*
	* Added by Jarno N. Alanko. Returns the smallest index j > i such that
	* BWT[j] = c. If such j does not exist, returns bwt_size()
	*/
	long long bwt_succ(ulint i, char c){
		ulint rank = bwt.rank(i+1,c);
		if(rank == bwt.rank(bwt_size(),c)) return bwt_size();
		return bwt.select(rank, uchar(c)); // rle_string select starts from 0
	}

	// Number of occurrences of c in BWT[0..i-1]
	long long bwt_rank(ulint i, char c){
		return bwt.rank(i,c);
	}
	...
	// Added by Jarno N. Alanko 23.5.2020.
	// Returns SA[i], where i is the smallest position larger or equal to lex_pos that is at a run end.
	ulint get_next_SA_sample(ulint lex_pos){
		ulint run_of_pos = bwt.run_of_position(lex_pos);
		ulint text_pos = samples_last[run_of_pos] + 1; // samples_last[run_of_pos] is SA[lex_pos of end of run]-1
		if(text_pos >= bwt_size()) text_pos = 0;
		return text_pos;
	}
...
private:
	 ...
	 */
	tuple<string, vector<pair<ulint, ulint> >, vector<ulint> > sufsort(string &s, std::function<void(int_vector_buffer<>& sa, int_vector<8>& T)> callback){
		...
		callback(sa,text); // Added by Jarno N. Alanko 24.5.2020.
	...
	// ^ Jarno N. Alanko note: they are not the text positions of ends of runs, but actually those decremented by 1 (wrapping around).
	// So they are the text positions after following the LF-mapping with that character
